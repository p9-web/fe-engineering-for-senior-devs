...
* **Parsing:** Arrange tokens into an **Abstract Syntax Tree** per the language grammar. `let x = 5` becomes (a *simplified* ESTree shape — real nodes from acorn/espree also carry `start`/`end`/`loc`, wrapped in a top-level `Program` node):
...
* **Why esbuild/SWC are fast:** written in Go/Rust, they parallelize across cores and do the whole pipeline in as few AST passes as possible — versus JS-based toolchains that re-traverse the tree many times. (More on judging this in Module 11.)