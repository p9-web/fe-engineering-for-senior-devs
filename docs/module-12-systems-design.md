...
* **Information hiding (Parnas):** decompose a system around the **decisions most likely to change**, not around the *steps* of a process. Hide the volatile thing — the storage format, the third-party API, the wire protocol — behind the seam, so changing it doesn't ripple.
...
JavaScript is single-threaded ([Module 1](/module-1-js-runtime)), but that does **not** make it race-free. Every `await` is a yield point where other code runs and shared state can change underneath you.
...
* When you *do* add real parallelism — Web Workers and `SharedArrayBuffer`/`Atomics` from [Module 10](/module-10-browser-apis) — the same invariant discipline scales up, except now the races are simultaneous and `Atomics` is your only safe read-modify-write.
...
* Distinguish **one-way doors from two-way doors** (Bezos): a reversible decision deserves a fast, cheap call; an irreversible one (a public API shape, a data model, a framework) deserves the ADR. Spend your judgment where it's expensive to undo.
