...
learn:
  ...
  recall:
    - "From memory: when an effect reads a signal, how does the signal wire that currently-running effect into its subscriber set so a later write can re-run it (Module 5)?"
    - "Before reading: why does moving a recycled row with transform: translateY stay on the compositor and skip layout and paint (Module 2)?"
    - "From memory: what does the module graph record about each file (Module 7) that lets the dev server rewrite a bare import specifier to a real URL?"
---
...
With fixed heights the spacer is `totalCount * rowHeight`. With variable heights, what replaces that single multiply?

<template #answer>

You build a *prefix-sum* (cumulative offset) array: reading row N's offset is then O(1), but finding *which* row sits at a given `scrollTop` becomes a **binary search** over that array — O(log n), not the O(1) division you had with fixed heights. Naming both costs is the point.
...
This is the reactive core *without* the `e.deps` cleanup above — deliberately naive. Predict all three console lines before you run it:

```js
let active = null
function signal(v) {
  const subs = new Set()
  return { get() { if (active) subs.add(active); return v }, set(n) { v = n; [...subs].forEach((f) => f()) } }
}
function effect(fn) { const run = () => { active = run; fn(); active = null }; run() }

const a = signal(1), b = signal(10)
let useA = true
effect(() => console.log('sum', useA ? a.get() : b.get()))
useA = false
b.set(20) // (1) does the effect re-run?
a.set(2)  // (2) does this re-run it?
```

<template #answer>

Logs `sum 1`, then **nothing** for `b.set(20)`, then `sum 20` for `a.set(2)`. On its first run the effect read only `a` (`useA` was true), so it subscribed to `a` alone. Flipping `useA` doesn't change subscriptions — this naive engine never unsubscribes. So updating `b` (which it *now* logically depends on) is ignored, while updating `a` (which it no longer uses) wrongly re-runs it — and *that* run reads `b`, finally printing `20`. This is exactly the stale-subscription bug the two-way `e.deps` link fixes: `run()` drops old subscriptions before re-tracking, so dependencies always match the latest read.
...
Your `effect` writes `node.nodeValue = count`. What happens to the user's text cursor / IME composition if you instead re-render by reassigning the parent's whole markup string?

<template #answer>

The node is destroyed and recreated, so focus and selection are lost — the reason fine-grained text patching exists.
...
A click handler does `count++; count++; count++`. With the batching above, how many times does a subscriber run, and on which turn of the event loop?

<template #answer>

Once, on the next microtask — the `Set` dedups the three invalidations of the same subscriber.
...
File `b.js` is imported by `a.js` and `c.js`. You edit `b.js`. Why does HMR need the *reverse* edges (importers), not the forward ones (imports), to know which modules to invalidate — and what stops the invalidation from cascading to the whole graph?

<template #answer>

You walk importers upward to find who's affected; the cascade stops at the first module that declares an HMR boundary — [`import.meta.hot.accept()`](https://vite.dev/guide/api-hmr.html#hot-accept-cb) — which says "I can swap my dependency in place, don't propagate past me." With no accepting boundary, it bubbles to the root and falls back to a full reload.
